...
// Response — always 200
{
  ...
  "cursor": 4831,
  "results": [
    { "op_id": "uuid", "status": "accepted", "server_values": { "table_session_id": 5031, "status": "open" } }
  ]
}
```

Per-operation `results[]` entries: `{ "op_id", "status": "accepted", "server_values": {...} }`, `{ "op_id", "status": "duplicate", "server_values": {...} }`, `{ "op_id", "status": "rejected", "retryable": false, "code": "validation_failed", "message": "..." }`, or `{ "op_id", "status": "deferred", "retryable": true, "code": "server_busy", "message": "..." }`. `server_values` is a direct passthrough of whatever hash the handler for that `type` returned (see `Api::V1::Sync::BatchController::TYPE_TO_KIND`, and each handler under `app/models/sync/handlers/`) — shape varies by `type`; it's how a client learns anything server-assigned (a real integer id, an invoice number, a print count) that its own payload couldn't carry. `type` → payload mapping (`table_session.open`, `ticket.create`, `ticket.status`, `ticket_item.void`, `table_session.discount`, `invoice.issue`, `payment.record`, `table_session.close`, `invoice.print`) is in the same constant — an unknown `type` comes back `rejected`/`validation_failed`, not a 4xx.

**`table_session.open` doesn't hand you a real id up front — it takes your own `client_token` instead.** Send a UUID you mint locally as `client_token` (not the payload's `id`, which the server never adopts as the row's id — see `API-SPEC.md` §5). The server finds-or-creates a session for that `dining_table_id` and returns the real integer id as `server_values.table_session_id`. From then on, use **either** that real id or your own `client_token` as `table_session_id` in every dependent op (`ticket.create`, `payment.record`, `invoice.issue`, `table_session.discount`, `table_session.close`) — the server tells them apart by shape (all-digits → real id, anything else → your `client_token`). Sending your own token throughout is the simpler, safer default for an offline-first client: it works even if you never see `table_session.open`'s response (dropped reply after a successful server-side write is exactly the case idempotency exists for). Two devices opening the *same* table is not an error either way — find-or-create means both land on the same session, `accepted`, no `rejected`/`session_exists` — that error code doesn't exist in this implementation despite an earlier spec draft describing it.
...
- **Menu item price ships as both `gross_price_rupees` (float, build your own cart/tax math on this) and `gross_price_paisa` (integer, for a client that wants exact integer math instead)** — pick one and don't mix them mid-calculation. This is a wire-format choice, not a server-internals one: the server's own storage and its own tax computation (`Billing.compute`) are integer paisa throughout regardless of which field a client used, `CLAUDE.md` invariant #1 governs that and hasn't changed. Elsewhere on the wire — `payment.record`'s `amount_paisa`, an offline invoice's `gross_paisa` — there is no rupee sibling; those stay integer paisa, no exception. When in doubt about a given field, check the controller — this guide documents what exists today, not a blanket rule to extrapolate from.
  **The mistake this section exists to prevent:** reading `gross_price_rupees` (already `180.0` for a Rs. 180 item) and then dividing it by 100 anyway, because the field name still has "gross_price" in it and old habit says "divide by 100." That produces `1.80`, silently, with no error anywhere — the server sent the right number, the client just applied a paisa→rupee conversion to a value that was already in rupees. `/100` belongs on `gross_price_paisa` only. If a displayed price looks 100x too small, check for exactly this before assuming the server is wrong.
- **`invoice.print` only records that a device already printed locally** (Bluetooth ESC/POS, or any client-local printer) — it just bumps `print_count`/writes an audit event. It never enqueues a server-side `PrintInvoiceJob`. That's a different path entirely (`reprint_invoice`, not exposed over `/sync/batch` today), used by the web admin to print via a network-attached printer (`Socket.tcp` to `shop.printer_host`/`printer_port`). Don't conflate the two — a device that already produced a physical receipt itself must send `invoice.print`, never `reprint_invoice`.
